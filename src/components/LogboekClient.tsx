...
export default function LogboekClient({ initial, phases }: { initial: LogEntry[]; phases: Phase[] }) {
  ...
  async function add(e: React.FormEvent) {
    ...
    if (data) {
      ...
      setError(insertError?.message ? "Opslaan mislukt — probeer het opnieuw." : "Opslaan mislukt — controleer je verbinding en probeer opnieuw.");
    }
    setSaving(false);
  }

  async function remove(id: string) {
    if (!confirm("Deze logboekregel verwijderen?")) return;
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
                ...
                {editId === it.id ? (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex shrink-0 items-center gap-0.5">
                      <button onClick={() => startEdit(it)} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-600" aria-label="Bewerken">
                      ...
                      <button onClick={() => remove(it.id)} className="rounded-lg p-2 text-slate-400 hover:bg-red-50 hover:text-red-500" aria-label="Verwijderen">
      ...
      {/* Formulier */}
      <form onSubmit={add} className="order-first h-fit space-y-3 rounded-2xl border border-slate-200 bg-white p-5 lg:order-none lg:sticky lg:top-20">
